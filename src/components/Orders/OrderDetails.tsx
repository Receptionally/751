...
export function OrderDetails({ order, showSellerInfo = false }: OrderDetailsProps) {
  const { charges } = useChargeHistory(order.seller_id);
  
  // Find subscription charge for this order
  const subscriptionCharge = charges?.find(
    charge => charge.metadata?.order_id === order.id
  );
  ...
  const sections = [
    ...
    subscriptionCharge && {
      title: 'Seller Subscription Charge',
      fields: [
        { 
          label: 'Amount', 
          value: formatCurrency(subscriptionCharge.amount / 100)
        },
        { 
          label: 'Status', 
          value: subscriptionCharge.status === 'succeeded' ? 'Paid' : 'Failed'
        },
        {
          label: 'Date',
          value: formatDate(new Date(subscriptionCharge.created * 1000))
        }
      ]
    },
    {
